...
class JdCutePet:
    ...
    async def request(self, session, function_id, body=None, wait_time=3):
        ...
        try:
            ...
            println('{}, 访问服务器异常:{}!'.format(self._pt_pin, e.args))
    ...
    async def init(self, session):
        ...
        if data['resultCode'] != '0':
            println('{}, 无法获取活动首页数据, {}!'.format(self._pt_pin, data['message']))
            return False

        if data['userStatus'] == 0:
            println('{}, 萌宠活动未开启, 请手动去京东APP开启活动入口：我的->游戏与互动->查看更多开启！'.format(self._pt_pin))
        ...
        println('{}的互助码为:{}'.format(self._pt_pin, self._share_code))
        println('{}的邀请码为:{}'.format(self._pt_pin, self._invite_code))
    ...
    async def sign(self, session, task):
        ...
        """
        if task['finished']:
            println('{}, 今日已完成签到!'.format(self._pt_pin))
            return
        ...
        if data['resultCode'] != '0':
            ...
            println('{}, 签到成功, 获得狗粮: {}g!'.format(self._pt_pin, data['signReward']))
    ...
    async def first_feed(self, session, task):
        ...
        """
        if task['finished']:
            println('{}, 今日已完成首次喂食任务!'.format(self._pt_pin))
            return

        data = await self.request(session, 'feedPets')

        if data['resultCode'] != '0':
            println('{}, 首次喂食任务失败!'.format(self._pt_pin))
        else:
            println('{}, 完成首次喂食任务, 获得狗粮: {}g!'.format(self._pt_pin, data['firstFeedReward']))
    ...
    async def three_meal(self, session, task):
        ...
        if not can_do or task['finished']:
            println('{}, 当前不在每日三餐任务时间范围内或者任务已完成!'.format(self._pt_pin))
            return

        data = await self.request(session, 'getThreeMealReward')

        if data['resultCode'] != '0':
            println('{}, 每日三餐任务完成失败!'.format(self._pt_pin))
        else:
            println('{}, 完成每日三餐任务成功, 获得狗粮:{}g!'.format(self._pt_pin, data['threeMealReward']))
    ...
    async def feed_food_again(self, session):
        ...
        :return:
        """
        println('{}, 再次进行喂食!'.format(self._pt_pin))
        data = await self.request(session, 'initPetTown')
        if data['resultCode'] != '0':
        ...
        if (int(food_amount) - keep_amount) > 10:
            ...
            for i in range(times):
                println('{}, 正在进行第{}次喂食!'.format(self._pt_pin, i+1))
                data = await self.request(session, 'feedPets')
                if data['resultCode'] != '0':
                    println('{}, 第{}次喂食失败, 停止喂食!'.format(self._pt_pin, i+1))
                else:
                    println('{}, 第{}次喂食成功!'.format(self._pt_pin, i+1))
        else:
            println('{}, 当前狗粮(剩余狗粮{}-保留狗粮{})不足10g ,无法喂食!'.format(self._pt_pin, food_amount, keep_amount))
    ...
    async def feed_reach_hundred(self, session, task):
        ...
        """
        if task['finished']:
            println('{}, 今日已完成喂狗粮{}g任务!'.format(self._pt_pin, task['feedReachAmount']))
            return

        had_feed_count = int(task['hadFeedAmount'] / 10)
        need_feed_count = int(task['feedReachAmount'] / 10)

        println('{}, 当前喂食任务进度:{}/{}!'.format(self._pt_pin, had_feed_count, need_feed_count))
        ...
        for i in range(had_feed_count, need_feed_count):
            ...
            if data['resultCode'] != '0':
                if data['resultCode'] == '3003':
                    println('{}, 第{}次喂食失败!, 狗粮剩余不足，退出喂食!'.format(self._pt_pin, i+1))
                    break
                else:
                    println('{}, 第{}次喂食失败!'.format(self._pt_pin, i + 1))
            else:
                println('{}, 第{}次喂食成功, 进度:{}/{}!'.format(self._pt_pin, i + 1, i + 1, need_feed_count))
    ...
    async def browser_task(self, session, task):
        ...
        """
        if task['finished']:
            println('{}, 今日已完成浏览任务:{}!'.format(self._pt_pin, task['title']))
            return
        ...
        if data['resultCode'] != '0':
            if data['resultCode'] == '5000':
                println('{}, 已领取浏览任务:{}!'.format(self._pt_pin, task['title']))
            else:
                println('{}, 领取浏览任务:{}失败!'.format(self._pt_pin, task['title']))
                return
        else:
            println('{}, 领取浏览任务:{}成功!'.format(self._pt_pin, task['title']))
        ...
        if data['resultCode'] != '0':
            println('{}, 完成浏览任务:{}失败!'.format(self._pt_pin, task['title']))
        else:
            println('{}, 完成浏览任务:{}成功, 获得{}g狗粮!'.format(self._pt_pin, task['title'], data['reward']))
    ...
    async def pet_sport(self, session):
        ...
        """
        times = 0
        println('{}, 正在遛狗...'.format(self._pt_pin))
        while True:
            times += 1
            ...
            if data['resultCode'] != '0':
                if data['resultCode'] == '1013':
                    println('{}, 第{}次遛狗失败, 有未领取奖励, 去领取!'.format(self._pt_pin, times))
                elif data['resultCode'] == '3001':
                    println('{}, 第{}次遛狗失败, 达到宠物运动次数上限!'.format(self._pt_pin, times))
                    break
                else:
                    println('{}, 第{}次遛狗失败, 结束遛狗!'.format(self._pt_pin, times))
                    break
            food_reward = data['foodReward']

            println('{}, 完成一次遛狗, 正在领取遛狗奖励!'.format(self._pt_pin))

            data = await self.request(session, 'getSportReward')

            if data['resultCode'] != '0':
                println('{}, 领取第{}次遛狗奖励失败, 结束遛狗!'.format(self._pt_pin, times))
            else:
                println('{}, 成功领取第{}次遛狗奖励, 获得狗粮:{}g, 当前狗粮:{}g!'.format(self._pt_pin, times,
    ...
    async def collect_energy(self, session):
        ...
        :return:
        """
        println('{}, 正在收取好感度...'.format(self._pt_pin))
    ...
    async def run(self):
        ...
        async with aiohttp.ClientSession(cookies=self._cookies, headers=self.headers) as session:
            ...
            if not is_success:
                println('{}, 初始化失败, 退出程序!'.format(self._pt_pin))
                return
