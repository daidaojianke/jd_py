...
class JdBurningSummer:
    ...
    code = None
    url = 'https://wbbny.m.jd.com/babelDiy/Zeus/2rtpffK8wqNyPBH6wyUDuBKoAbCt/index.html'
    browser = None
    page = None
    browser_cookies = []
    ...
    async def query_task_result(self, session, task_token):
        ...
        try:
            ...
            println('{}, 查询任务结果失败:{}'.format(self.account, e.args))
    ...
    async def do_task(self, session, task, view_page=False, action_type=1):
        ...
        for item in task_item_list:
            ...
            println(res)
            if res['bizCode'] == 0:
                println('{}, 成功领取任务:《{}》!'.format(self.account, title))
            else:
                println('{}, 无法领取任务:《{}》!'.format(self.account, title))
                continue

            if not view_page:  # 不需要需要访问页面
                if res['bizCode'] != 0:
                    println('{}, 任务:《{}》执行失败, {}'.format(self.account, title, res['bizMsg']))
                else:
                    println('{}, 完成任务:《{}》, 获得卡币:{}!'.format(self.account, title, res['result']['score']))
                continue

            success = await self.browser_task_page_view(session, item['taskToken'])
            if success:
                println('{}, 成功完成任务《{}》!'.format(self.account, title))
            else:
                println('{}, 无法完成任务《{}》!'.format(self.account, title))

            println('{}, 正在进行任务:《{}》, 需要等待{}秒!'.format(self.account, title, timeout))
            await asyncio.sleep(timeout)

            res = await self.query_task_result(session, item['taskToken'])
            if res['code'] == '0':
                println('{}, 任务:{}, {}'.format(self.account, title, res['toast']['subTitle']))
                continue
            else:
                println('{}, 无法领取任务:《》的奖励!'.format(self.account, title))
    ...
    async def get_lottery_shop_sign(self, session):
        ...
        if not data or data['code'] != '0':
            println('{}, 无法获取免费抽奖数据!'.format(self.account))
            return None
    ...
    async def receive_coupon_currency(self, session):
        ...
        # if data['code'] != '0':
        #     println('{}, 获取优惠券列表失败!'.format(self.account))
        #     return
        ...
        # for item in item_list:
            ...
            # println('{}, 领取优惠券《{}》, {}'.format(self.account, coupon_name, res['subCodeMsg']))
            # await asyncio.sleep(1)
            # data = await self.request(session, 'olympicgames_collectCurrency', {'type': 4})
            # if data['bizCode'] == 0:
            #     println('{}, 成功领取卡币, 当前卡币:{}!'.format(self.account, data['result']['poolCurrency']))
            # else:
            #     println('{}, 领取优惠券任务的卡币失败, {}'.format(self.account, data['bizMsg']))
            #     if data['bizCode'] == -902:
            #         break
            # await asyncio.sleep(1)
    ...
    async def receive_cash(self, session):
        ...
        if data['bizCode'] != 0:
            println('{}, 领红包失败, {}!'.format(self.account, data['bizMsg']))
            return
        println('{}, 成功领取红包, 当前红包总额:{}!'.format(self.account, data['result']['userActBaseVO']['totalMoney"']))
    ...
    async def do_sport(self, session):
        ...
        if data['bizCode'] != 0:
            println('{}, 无法进行运动, {}!'.format(self.account, data['bizMsg']))
        ...
        while True:
            ...
            if data['bizCode'] != 0:
                println('{}, 运动加速失败, {}!'.format(self.account, data['bizMsg']))
                break
            else:
                println('{}, 运动加速中!'.format(self.account))

            println('{}, 等等1s, 进行下一次运动加速!'.format(self.account))
            await asyncio.sleep(1)

        data = await self.request(session, 'olympicgames_endTraining')
        if data['bizCode'] != 0:
            println('{}, {}!'.format(self.account, data['bizMsg']))
        else:
            println('{}, 完成运动, 获得卡币:{}!'.format(self.account, data['result']['currencyNum']))
    ...
    async def collect_currency(self, session):
        ...
        if data['bizCode'] != 0:
            println('{}, 收取任务卡币失败:{}'.format(self.account, data['bizMsg']))
        else:
            println('{}, 成功收取任务卡币, 当前卡币:{}!'.format(self.account, data['result']['poolCurrency']))

        data = await self.request(session, 'olympicgames_collectCurrency', {"type": 2})
        if data['bizCode'] != 0:
            println('{}, 收取运动卡币失败:{}'.format(self.account, data['bizMsg']))
        else:
            println('{}, 成功收取运动卡币, 当前卡币:{}!'.format(self.account, data['result']['poolCurrency']))
    ...
    async def do_tasks(self, session, app_sign="1"):
        ...
        if app_sign == '1':
            # 邀请码
            self.code = data['result']['inviteId']
            channel = '京东APP'
        else:
            channel = '微信小程序'
        task_list = data['result']['taskVos']
        println('{}, 开始执行{}-每日任务...'.format(self.account, channel))
        ...
        for task in task_list:
            ...
            #     await self.shopping_task(session, task)
                ...
                println('{}, 任务《{}》暂未实现!'.format(self.account, task['taskType']))
    ...
    async def lottery(self, session, channel_sign="1", shop_sign="1000014803"):
        ...
        for task in task_list:
            ...
            if task_status == 2:
                println('{}, 免费抽奖任务《{}》今日已完成!'.format(self.account, task_name))
                continue

            if task_type in [14, 21]:
                println('{}, 免费抽奖任务《{}》跳过!'.format(self.account, task_name))
                continue
            ...
            if res['bizCode'] == 0:
                println('{}, 成功完成免费抽奖任务:《{}》!'.format(self.account, task_name))
            else:
                println('{}, 无法完成免费抽奖任务:《{}》!'.format(self.account, task_name))

        data = await self.request(session, 'olympicgames_shopLotteryInfo',
                                  {"channelSign": channel_sign, "shopSign": shop_sign})

        if data['bizCode'] != 0:
            println('{}, 无法查询当前抽奖次数!'.format(self.account))
            return
        lottery_num = data['result']['boxLotteryNum']

        if lottery_num < 1:
            println('{}, 当前抽奖次数不足!'.format(self.account))
            return

        println('{}, 开始执行免费抽奖, 抽奖次数:{}!'.format(self.account, lottery_num))

        for i in range(lottery_num):
            res = await self.request(session, 'olympicgames_boxShopLottery', {"shopSign": shop_sign})
            if res['bizCode'] != 0:
                println('{}, 第{}次免费抽奖失败!'.format(self.account, i + 1))
            else:
                println('{}, 第{}次免费抽奖成功!'.format(self.account, i + 1))
    ...
    async def indiana(self, session):
        ...
        if data['bizCode'] != 0:
            println('{}, 无法获取每日夺宝数据!'.format(self.account))
            return

        item_list = data['result']['pawnshopInfo']['betGoodsList']
        for item in item_list:
            if item['score'] > 0:
                println('{}, 夺宝:{}, 已下注!'.format(self.account, item['skuName']))
                continue
            res = await self.request(session, 'olympicgames_pawnshopBet', {"skuId": item['skuId']})

            if res['bizCode'] == 0:
                println('{}, 夺宝:{}, 成功下注!'.format(self.account, item['skuName']))
            else:
                println('{}, 夺宝:{}, 下注失败, {}!'.format(self.account, item['skuName'], res['bizMsg']))
    ...
    async def shopping_task(self, session, task):
        ...
        for item in item_list:
            ...
            res = await self.request(session, 'olympicgames_doTaskDetail', body)
            if res['bizCode'] != 0:
                println('{}, 加购失败, {}!'.format(self.account, res['bizMsg']))
                break
            println('{}, 完成一次加购任务!'.format(self.account))
        ...
        for code in JD_BURNING_SUMMER_CODE:
            ...
            if res['bizCode'] == 0:
                if 'hongBaoVO' in res['result']:
                    ...
                    award = ''
                println('{}, 成功助力好友, 奖励: {}!'.format(self.account, award))
            else:
                println('{}, 助力好友失败, {}!'.format(self.account, res['bizMsg']))

    @logger.catch
    async def login(self, session):
        data = await self.request(session, 'olympicgames_home')
        if data['bizCode'] != 0:
            println('{}, 获取活动首页数据失败!'.format(self.account))
            return False
        println('{}, 已连续登录:{}天!'.format(self.account, data['result']['continuedSignDays']))
        return True
    ...
    async def get_ss(self):
        ...
        获取验证参数
        """
        println('{}, 正在获取验证参数!'.format(self.account))
        try:
    ...
    async def run(self):
        ...
        async with aiohttp.ClientSession(headers=self.headers, cookies=self.cookies) as session:
            ...
            if not success:
                println('{}, 无法登录活动首页, 未开启活动或账号已黑!'.format(self.account))
                return
...
if __name__ == '__main__':
    ...
    process_start(JdBurningSummer, '燃动夏季')
